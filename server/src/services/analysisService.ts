...
import * as analysisGuard from '../middleware/analysisGuard.js';
...
export async function generateAnalysis(portfolioId: number): Promise<PortfolioAnalysis> {
  ...
  // Atomic check-and-track to prevent race conditions
  if (!analysisGuard.checkAndTrack(portfolioId)) {
    throw new AppError(409, 'Analysis is already in progress for this portfolio');
  }

  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) {
    analysisGuard.cleanupAnalysis(portfolioId);
    throw new AppError(500, 'ANTHROPIC_API_KEY not configured on server');
  }

  // Track this analysis to prevent concurrent runs
  ...
  // Track the analysis
  analysisGuard.trackAnalysis(portfolioId, analysisPromise);

  try {
    await analysisPromise;
  } finally {
    // Always clean up tracking, even if analysis fails
    analysisGuard.cleanupAnalysis(portfolioId);
  }
